...
| Tab switch | Applies the exact selected tab and URL color immediately, then requests fresh page pixels. Broader page and same-host colors remain delayed fallbacks. Color transitions are enabled only while the selected page is actively loading; switching tabs clears the loading transition before applying the next tab's color. Equivalent color keys are no-ops to avoid needless chrome writes. |
